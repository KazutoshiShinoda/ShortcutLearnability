...
def convert_examples_to_features(
    ...
    for (ex_index, example) in tqdm.tqdm(enumerate(examples), desc="convert examples to features"):
        ...
        if task == 'race':
            example_id = example.example_id + example.question + ''.join(example.endings)
        else:
    ...
    if num_truncated_examples > 0:
        logger.info(
            f"{num_truncated_examples} examples are truncated! (swag task is ok). "
...
processors = {"race": RaceProcessor, "swag": SwagProcessor, "arc": ArcProcessor, "syn": SynonymProcessor, "reclor": ReclorProcessor}
MULTIPLE_CHOICE_TASKS_NUM_LABELS = {"race": 4, "swag": 4, "arc": 4, "syn": 5, "reclor": 4}
